/*
 SnailJob 2.0.2 PostgreSQL 初始化脚本。
 来源：RuoYi-Vue-Plus 6.x 官方 PostgreSQL 脚本，按 Zero Admin 的
 local/dev/prod 命名空间、客户端组名和安全占位令牌进行适配。

 仅在首次启用任务调度中心时执行一次，不可作为幂等迁移重复执行。
*/
...
INSERT INTO sj_namespace VALUES (1, 'Local', 'local', '', 0, now(), now());
INSERT INTO sj_namespace VALUES (2, 'Development', 'dev', '', 0, now(), now());
INSERT INTO sj_namespace VALUES (3, 'Production', 'prod', '', 0, now(), now());
...
INSERT INTO sj_group_config VALUES (1, 'local', 'zero_admin_group', '', 'change-me-before-enabling', 1, 1, 0, 1, 1, now(), now());
INSERT INTO sj_group_config VALUES (2, 'dev', 'zero_admin_group', '', 'change-me-before-enabling', 1, 1, 0, 1, 1, now(), now());
INSERT INTO sj_group_config VALUES (3, 'prod', 'zero_admin_group', '', 'change-me-before-enabling', 1, 1, 0, 1, 1, now(), now());
...
-- 显式插入主键后同步序列，避免控制台新增数据时发生主键冲突。
SELECT setval(pg_get_serial_sequence('sj_namespace', 'id'), (SELECT max(id) FROM sj_namespace));
SELECT setval(pg_get_serial_sequence('sj_group_config', 'id'), (SELECT max(id) FROM sj_group_config));
